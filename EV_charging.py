...
# Predefined city coordinates for plotting
city_coordinates = {
    "Toronto": [43.7, -79.42],
...
# Create a base map centered on Canada with appropriate zoom level
canada_map = folium.Map(location=[56.1304, -106.3468], zoom_start=4)

# Load the electrical stations location data from the uploaded file
charging_station_data = pd.read_csv("electrical_stations_locations.csv")

# Add smaller markers for each electrical station with a distinct color
for _, row in charging_station_data.iterrows():
    folium.CircleMarker(
        location=[row["Latitude"], row["Longitude"]],
        radius=5,  # Adjust the radius to make the marker smaller or larger
        color="red",
        fill=True,
        fill_color="red",
        fill_opacity=0.7,
...
# Add HeatMap layer (placing it after the markers so it shows on top)
HeatMap(heat_data).add_to(canada_map)

# Save the final map to an HTML file
canada_map.save("canada_aqi_and_stations_map_with_locations.html")

print("Heatmap with charging station markers saved as 'canada_aqi_and_stations_map_with_locations.html'. Open this file in a web browser to view the map.")

# open -a "Google Chrome" canada_aqi_and_stations_map_with_locations.html
