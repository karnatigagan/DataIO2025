...
# Load your Global Air Pollution dataset
global_air_pollution = pd.read_csv("global air pollution dataset.csv")

# Calculate the average PM2.5 for each city
average_pm25_per_city = global_air_pollution.groupby("City")["PM2.5 AQI Value"].mean().reset_index()

# Predefined city coordinates for plotting (you can adjust or add more cities)
...
average_pm25_per_city["Latitude"] = average_pm25_per_city["City"].map(lambda x: city_coordinates.get(x, [None, None])[0])
average_pm25_per_city["Longitude"] = average_pm25_per_city["City"].map(lambda x: city_coordinates.get(x, [None, None])[1])

# Remove rows with missing coordinates
average_pm25_per_city = average_pm25_per_city.dropna(subset=["Latitude", "Longitude"])

# Create a base map centered on Canada
canada_map = folium.Map(location=[56.1304, -106.3468], zoom_start=4)

# Prepare heatmap data (latitude, longitude, PM2.5 AQI Value)
heat_data = list(zip(
    average_pm25_per_city["Latitude"],
    average_pm25_per_city["Longitude"],
    average_pm25_per_city["PM2.5 AQI Value"]
))
...
canada_map.save("canada_pm25_heatmap.html")

print("Heatmap saved as 'canada_pm25_heatmap.html'. Open this file in a web browser to view the heatmap.")

# open -a "Google Chrome" canada_pm25_heatmap.html